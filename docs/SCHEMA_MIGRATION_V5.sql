-- Schema Migration V5: Separate sandbox and production skiptrace tables
-- Run this in your Supabase SQL Editor

-- Step 1: Drop existing tables (WARNING: This will delete all data!)
DROP TABLE IF EXISTS skiptrace_sandbox CASCADE;
DROP TABLE IF EXISTS skiptrace CASCADE;
DROP TABLE IF EXISTS defendants CASCADE;
DROP TABLE IF EXISTS cases CASCADE;

-- Step 2: Create new schema with separate skiptrace tables
...
-- Create skiptrace table for PRODUCTION API results
...
-- Create skiptrace_sandbox table for SANDBOX API testing
CREATE TABLE skiptrace_sandbox (
    id SERIAL PRIMARY KEY,
    docket_number VARCHAR(100) NOT NULL REFERENCES cases(docket_number) ON DELETE CASCADE,
    phone_number VARCHAR(20),
    phone_type VARCHAR(50),
...
CREATE INDEX idx_skiptrace_sandbox_docket ON skiptrace_sandbox(docket_number);
CREATE INDEX idx_skiptrace_sandbox_phone ON skiptrace_sandbox(phone_number);